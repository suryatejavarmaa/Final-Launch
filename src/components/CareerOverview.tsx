    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10" ref={containerRef}>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-stretch max-w-5xl mx-auto">
          ...
          <ElectricBorder color="#E11D48" speed={0.5} chaos={0.15} borderRadius={32} className="h-full">
            <div
              ...
              <div className="relative z-10 flex-1 flex flex-col">
                <div className="mb-auto">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, rgba(225, 29, 72, 0.3) 0%, rgba(225, 29, 72, 0.1) 100%)', border: '1px solid rgba(225, 29, 72, 0.4)', boxShadow: '0 0 20px rgba(225, 29, 72, 0.3)', padding: '8px' }}>
                      <Rocket className="w-6 h-6 text-white" />
                    </div>
                    <div className="px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, rgba(225, 29, 72, 0.2) 0%, rgba(225, 29, 72, 0.05) 100%)', border: '1px solid rgba(225, 29, 72, 0.3)' }}>
                      <span className="text-xs font-semibold text-white tracking-wider uppercase">Entrepreneur Path</span>
                    </div>
                  </div>
                  <h4 className="text-3xl lg:text-4xl font-black mb-4 tracking-tight" style={{ color: '#F43F5E' }}>Create Your Empire</h4>
                  <p className="text-slate-400 text-base lg:text-lg leading-relaxed max-w-sm">Launch and scale your own startup with expert guidance and proven frameworks.</p>
                </div>
                <div className="space-y-3 mt-8">
                  {["Startup toolkit & resources", "Mentor network access", "Funding & pitch guidance"].map((item, i) => (
                    <div key={i} className="group flex items-center gap-4 py-3 px-4 rounded-2xl transition-all duration-300 hover:translate-x-1" style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
          ...
          {/* Career Path - BLUE */}
          <ElectricBorder color="#0EA5E9" speed={0.5} chaos={0.15} borderRadius={32} className="h-full">
            <div
              className="h-full min-h-[420px] rounded-[32px] p-8 lg:p-10 relative overflow-hidden flex flex-col"
              style={{
            ...
            >
              ...
              <div className="relative z-10 flex-1 flex flex-col">
                <div className="mb-auto">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, rgba(14, 165, 233, 0.3) 0%, rgba(14, 165, 233, 0.1) 100%)', border: '1px solid rgba(14, 165, 233, 0.4)', boxShadow: '0 0 20px rgba(14, 165, 233, 0.3)', padding: '8px' }}>
                      <Briefcase className="w-6 h-6 text-white" />
                    </div>
                    <div className="px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, rgba(14, 165, 233, 0.2) 0%, rgba(14, 165, 233, 0.05) 100%)', border: '1px solid rgba(14, 165, 233, 0.3)' }}>
                      <span className="text-xs font-semibold text-white tracking-wider uppercase">Career Path</span>
                    </div>
                  </div>
                  <h4 className="text-3xl lg:text-4xl font-black mb-4 tracking-tight" style={{ color: '#38BDF8' }}>Build Your Future</h4>
                  <p className="text-slate-400 text-base lg:text-lg leading-relaxed max-w-sm">Master skills and land your dream job in tech with industry-ready training.</p>
                </div>
                <div className="space-y-3 mt-8">
                  {["Industry-grade projects", "Technical interview prep", "Job placement support"].map((item, i) => (
                    <div key={i} className="group flex items-center gap-4 py-3 px-4 rounded-2xl transition-all duration-300 hover:translate-x-1" style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
